...
export const columns: Array<ColumnDef<Partner>> = [
    {
        accessorKey: 'id',
                ...
                <>
                    <DropdownMenu modal={false}>
                        ...
                        <DropdownMenuContent align="end" className="w-40">
                            ...
                            <DropdownMenuGroup>
                                <DropdownMenuItem disabled={!can('supplier.show')} asChild>
                                    <Link
                                        className="block w-full"
                                        href={erp.suppliers.show(row.original.id)}
                                        as="button"
                                    >
                                        <EyeIcon aria-hidden size={14} /> View
                                    </Link>
                                </DropdownMenuItem>
                                <DropdownMenuItem disabled={!can('supplier.edit')} asChild>
                                    <Link
                                        className="block w-full"
                                        href={erp.suppliers.edit(row.original.id)}
                                        as="button"
                                    >
                    ...
                    <ActionConfirmationDialog
                        ...
                        description={`Are you sure you want to soft delete supplier "${row.original.name}"? This action can be undone later.`}
                        method="delete"
                        route={erp.suppliers.destroy(row.original.id)}
                    />
                </>
            );
        },
